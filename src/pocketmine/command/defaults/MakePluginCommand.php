...
class MakePluginCommand extends VanillaCommand{
	...
	public function execute(CommandSender $sender, $commandLabel, array $args){
        ...
        // TODO: Does not work because pocketmine\plugin\FolderPluginLoader does not exist, maybe copy the file from DevTools
        //if(!($plugin->getPluginLoader() instanceof FolderPluginLoader)){
		//	$sender->sendMessage(TextFormat::RED . "Plugin ".$description->getName()." is not in folder structure.");
        //	return true;
		//}

        $devPath = Server::getInstance()->getPluginPath() . "ImagicalMine" . DIRECTORY_SEPARATOR;
		$pharPath = $devPath . $description->getName()."_v".$description->getVersion().".phar";
		...
		foreach(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($filePath)) as $file){
			...
			$sender->sendMessage("[ImagicalMine] Adding $path");
		}
