...
class AutoUpdater{
	...
	public function showConsoleUpdate(){
		...
		$logger->warning("----- ImagicalMine Auto Updater -----");
		$logger->warning("Your version of ImagicalMine is out of date. Version " . $newVersion->get(false) . " (build #" . $newVersion->getBuild() . ") was released on " . date("D M j h:i:s Y", $this->updateInfo["date"]));
	...
	public function showPlayerUpdate(Player $player){
		$player->sendMessage(TextFormat::DARK_PURPLE . "The version of ImagicalMine that this server is running is out of date. Please consider updating to the latest version.");
	...
	protected function showChannelSuggestionStable(){
		$logger = $this->server->getLogger();
		$logger->info("----- ImagicalMine Auto Updater -----");
	...
	protected function showChannelSuggestionBeta(){
		...
		$logger->info("If you would like to be kept informed about new Beta or Development builds, it is recommended that you change 'preferred-channel' in your imagicalmine.yml to 'beta' or 'development'.");
		$logger->info("----- -------------------------- -----");
	}
